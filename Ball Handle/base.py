    ...
    def run(self):
        while(True):
            if not imuQueue.full() : #and int-pin imu auslesen
                ...
                imuQueue.put((1000,0)) #TEST DATA
            if not camQueue.full():
                #camQueue.put(camQueue.process())
                camQueue.put((100,0))  #TEST DATA
...
class Processing(threading.Thread):
    ...
    def observer(self):
        ...
        self.impact_point = (DRIBBEL_POINT_X, impact_Y())
        self.ang2impact_left, dump = tangent_point(SERVO_POS_LEFT, self.impact_point)
        dump, self.ang2impact_right = tangent_point(SERVO_POS_RIGHT, self.impact_point)
    ...
    def controller(self):
        ...
        def wheel_velocity(data_ball_in):
            ball_mag, ball_ang = data_ball_in
            v_left = -(ball_mag * (
        ...
        def accept_ball():
            ...
            self.V_set_left, self.V_set_right = wheel_velocity(self.cart2pol((self.ball_set.V_X,self.ball_set.V_Y), self.impact_point))
        def dribbel_ball():
            # 1. fix the servos to ideal ball handle position
            self.ang_set_left = ANG_DRIBBEL_LEFT
            self.ang_set_right = ANG_DRIBBEL_RIGHT
        ...
        if ball_status_new is FAR_BALL:
            ball_status_old = FAR_BALL
            return
        if ball_status_new is NEAR_BALL:
            accept_ball()
            ball_status_old = NEAR_BALL
            return
        if ball_status_new is HAVE_BALL:
            dribbel_ball()
            ball_status_old = HAVE_BALL
            return

    def run(self):
        while(True):
            #read sensordata
            if not Input.imu.Queue.empty() : #and int-pin imu auslesen
                self.robot.V_X, self.robot.V_Y, self.robot.w_Z = Input.imu.Queue.get()
            if not Input.cam.Queue.empty():
                self.ball_measure.V_X, self.ball_measure.V_Y = Input.cam.Queue.get()
            ...
            '''
            # set actuatordata
            if not servo_leftQueue.full():
                servo_leftQueue.put(self.ang_left)
            if not servo_rightQueue.full():
                servo_rightQueue.put(self.ang_right)
            if not wheel_leftQueue.full():
                wheel_leftQueue.put(self.V_left)
            if not wheel_rightQueue.full():
                wheel_rightQueue.put(self.V_right)
            '''
    ...
    def run(self):
        while(True):
            if not outputQueue.empty():
                ...
                print("self.servo_left.ang_set, self.servo_right.ang_set, self.wheel_left.V_set, self.wheel_right.V_set",self.servo_left.ang_set, self.servo_right.ang_set, self.wheel_left.V_set, self.wheel_right.V_set)
