...
class Servo(object):
    def __init__(self, name, port, ang_min, ang_max, ang_crit, ang_start, ang_dribbel,
        ...
        self.pwm_crit = abs(ang_crit * self.pwm_factor) + self.pwm_min
        self.ang_set = 0  #self.ang_dribbel
    ...
    def hand_control(self):
        ang = input("set angle (in deg, 0...180): ")
...
servo_left = Servo(sim_mode=False, name='left', port="P9_14", ang_min=40, ang_max=125, ang_crit=SERVO_ANG_CRIT_LEFT,
                   ang_start=SERVO_ANG_START, ang_dribbel=SERVO_ANG_DRIBBEL_LEFT,  pwm_min=5, pwm_max=10, start_duty=7.75,
                   ang_offset=SERVO_ANG_OFFSET_LEFT, p_x=30, p_y=-100)

servo_right = Servo(sim_mode=False, name='right', port="P9_16", ang_min=-125, ang_max=-40, ang_crit=SERVO_ANG_OFFSET_RIGHT,
